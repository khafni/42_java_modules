    ...
    public void update(Message message) {
        try (Connection conn = dataSource.getConnection();
             ...
             var checkRoomIdPs = conn.prepareStatement("""
                     ...
                     WHERE id = ?
                     """);
             var updateMessage = conn.prepareStatement("""
                     UPDATE message
            ...
            if (isChatRoomIdValid.getInt(1) == 0)
                throw new NotSavedSubEntityException("chatRoom has no ID assigned in the database");
            if (findById(message.getId()).isPresent() == false)
                throw new NotSavedSubEntityException("the message id is invalid");
            updateMessage.setLong(1, message.getAuthor().getId());
            updateMessage.setLong(2, message.getRoom().getId());
            updateMessage.setString(3, message.getText());
            updateMessage.setTimestamp(4, message.getDate() == null? null: Timestamp.valueOf(message.getDate()));
            updateMessage.setLong(5, message.getId());
